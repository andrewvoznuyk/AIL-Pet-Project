...
const PlaneSelectForm = () => {
  ...
  const SendData = (event) => {
    ...
    setData({
      ...data,
      model: "/api/aircraft-models/" + currentPlane.id,
      company: "/api/companies/" + currentCompany.id
    });
        ...
        <FormControl style={{ width: 600 }}>
          ...
          >
            {planes && planes.map((item, key) => (
              <MenuItem
                key={key} value={item.plane} onClick={() => {
                setCurrentPlane(item);
                setData({ ...data, model: "/api/aircraft-models/" + item.id });
              }}
              >{item.plane}</MenuItem>
            ))}
          </Select>
