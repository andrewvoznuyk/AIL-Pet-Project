...
const PlaneForm = ({ currentPlane, data, setData }) => {
  ...
  useEffect(() => {
    setData({
      ...data,
      places: {
        "econom": parseInt(places.economPlaces),
        "standard": parseInt(places.standardPlaces),
        "business": parseInt(places.businessPlaces)
      }
    });
  }, [places]);
    ...
    <>
      ...
      <div>
        <TextField
          ...
          setPlaces({
            ...places,
            businessPlaces: event.target.value
          });
        }} style={{ margin: 10 }}
        />
        <TextField
          label="Econom places" type="number" id="econom" name="econom" defaultValue={places.economPlaces} onChange={(event) => {
          setPlaces({
            ...places,
            economPlaces: event.target.value
          });
        }} style={{ margin: 10 }}
        />
        <TextField
          label="Standard places" type="number" id="standard" name="standard" defaultValue={places.standardPlaces} onChange={(event) => {
          setPlaces({
            ...places,
            standardPlaces: event.target.value
          });
        }} style={{ margin: 10 }}
        />
      </div>
      <TextField
        label="Input plane number" type="text" id="planeNum" name="planeNum" onChange={(event) => {
        setData({
          ...data,
